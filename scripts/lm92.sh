...
for t in $(seq 1 1)
do
	check_range -b ${base} -d 50 -r -v -w 2 temp${t}_min
	rv=$(($? + ${rv}))
	check_range -b ${base} -d 50 -r -v -w 2 temp${t}_max
	rv=$(($? + ${rv}))
	check_range -b ${base} -d 50 -r -v -w 2 temp${t}_crit
	rv=$(($? + ${rv}))
	check_range -b ${base} -d 50 -r -v -w 2 temp${t}_crit_hyst
	rv=$(($? + ${rv}))
done
